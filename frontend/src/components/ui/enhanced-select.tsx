...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none focus:bg-slate-100 focus:text-slate-900 hover:bg-slate-50 data-[disabled]:pointer-events-none data-[disabled]:opacity-50 transition-colors duration-150",
      className
    )}
    {...props}
  >
...
>(({ value, onValueChange, placeholder, children, className, disabled, ...props }, _ref) => {
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Handle first letter typing
      if (event.key.length === 1 && /[a-zA-Z]/.test(event.key)) {
        event.preventDefault()
        setSearchTerm(prev => prev + event.key.toLowerCase())
        
        // Find and select the first item that starts with the typed letter
        ...
        if (matchingItem) {
          (matchingItem as HTMLElement).click()
        }
      }
